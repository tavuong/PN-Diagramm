...
def main():
    ...
    # command python z-tranform_2021.py factor
    # command python z-tranform_2021.py '0.7'
...
# ----PLOT ------------------
    plotz("z(jωT)", 100,factor, 0, 360,point_zero, point_pole,POINT_NUMBER, z_circle)
    draw_PNpoints (point_zero, point_pole, POINT_NUMBER)
    plotz("H(z) funtion", 100,factor, 0, 360,point_zero, point_pole,POINT_NUMBER, z_function)
    plotz("abs H(z)", 150,factor,  0, 360,point_zero, point_pole,POINT_NUMBER, z_abs)
...
def plotz(title, radius, factor, start_degrees, end_degrees, point_zero, point_pole,POINT_NUMBER, function):
# polar plot for z-varaiable and z-funktion
    ...
    for degrees in range(start_degrees, end_degrees + 1):
        ...
        pos = function(point_zero, point_pole,POINT_NUMBER,radians, radius,factor)
...
def z_circle(point_zero, point_pole,POINT_NUMBER,radians, radius,factor):
...
def z_function(point_zero, point_pole,POINT_NUMBER, radians, radius,factor):
...
#   PN position calculate 
   ...
   # H(z) == z_h1 / z_h2
    ...
    if abs(z_h2) != 0:
        ...
        distance = DIS_MAX
    # skalierung für |H(z)| 
    distance = distance * radius*factor
...
def z_abs(point_zero, point_pole,POINT_NUMBER,radians, radius,factor):
